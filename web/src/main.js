...
async function runStepTest() {
  ...
  if (rpm.length < 10) return "Not enough samples. Is the simulation paused?";
...
async function runAutoTune() {
  ...
  if (rpm.length < 20) return "Not enough samples. Is the simulation paused?";
  const final = rpm.slice(-10).reduce((a, v) => a + v, 0) / 10;
  if (Math.abs(final) < 10)
    return "The motor did not move. Check the drive settings.";
...
async function startReplay(name) {
  ...
  show();
  note(`Replaying "${name}". The scene is showing the recording, not live data`);
}
...
<p class="meta">Generated by the Motor Test Bench, developed by Vibodh Ayyapureddi. Print this page to save it as a PDF.</p>
</body></html>`;
  ...
  note("Report downloaded. Open it and print to PDF if you want one");
}
...
socket.on("status", ({ connected }) => {
  ...
  if (!connected) note("Connection lost, reconnecting", 0);
  else note("");
});
    ...
    case "scenario_step":
      ...
      if (bench === activeBench)
        panels.setScenarioStatus(
          `${msg.name}: step ${msg.index} of ${msg.total}, ${msg.label} at ${msg.t}s`);
      break;
